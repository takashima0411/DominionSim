...
//          if (players.get(0).pprUsed)
    ...
    private String getLandMarkText(DomPlayer thePlayer) {
        ...
        if (getBoard().isLandmarkActive(DomCardName.WolfDen)) {
            theBuilder.append("(").append(DomCardName.WolfDen.toHTML()).append(":").append(WolfDenCard.countVP(thePlayer)).append("&#x25BC;) ");
        }
        ...
        if (getBoard().isLandmarkActive(DomCardName.TriumphalArch)) {
            theBuilder.append("(").append(DomCardName.TriumphalArch.toHTML()).append(":").append(TriumphalArchCard.countVP(thePlayer)).append("&#x25BC;) ");
        }
        ...
        if (getBoard().isLandmarkActive(DomCardName.BanditFort)) {
            theBuilder.append("(").append(DomCardName.BanditFort.toHTML()).append(":").append(BanditFortCard.countVP(thePlayer)).append("&#x25BC;) ");
        }
    ...
    public int getBridgeTrollsInPlay() {
        return activePlayer.getCardsFromPlay(DomCardName.BridgeTroll).size();
    }
    ...
    public boolean isSwampHagActive() {
        if (!isInKingDom(DomCardName.SwampHag))
            return false;
        for (DomPlayer theOpp : activePlayer.getOpponents()) {
            if (!theOpp.getCardsFromPlay(DomCardName.SwampHag).isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    public boolean checkForMountainPass() {
        if (isNoProvinceGainedYet && getBoard().isLandmarkActive(DomCardName.MountainPass)) {
