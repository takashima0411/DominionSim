...
public class GearCard extends DomCard {
    ...
    public void play() {
        ...
        while ((owner.removingReducesBuyingPower(theTreasures.get(i)) || theTreasures.get(i).getName() == DomCardName.HornOfPlenty) && i > 0)
            i--;
        if (owner.removingReducesBuyingPower(theTreasures.get(i)) || theTreasures.get(i).getName() == DomCardName.HornOfPlenty) {
            checkForOtherJunk();
            return;
        }
