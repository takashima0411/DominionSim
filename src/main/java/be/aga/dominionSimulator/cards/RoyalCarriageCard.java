...
public class RoyalCarriageCard extends DomCard {
    private DomCard myDurationCard;

    public RoyalCarriageCard() {
        super(DomCardName.RoyalCarriage);
    }
    ...
    public void doWhenCalled() {
        ...
        for (i = owner.getCardsInPlay().size() - 1; i >= 0; i--) {
            if (owner.getCardsInPlay().get(i).getName() == DomCardName.RoyalCarriage)
                continue;
            break;
        }
