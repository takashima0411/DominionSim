...
public class ProvinceCard extends DomCard {
    ...
    private void handleFoolsGold() {
        for (DomPlayer thePlayer : owner.getOpponents()) {
            ...
            if (thePlayer.getCardsFromHand(DomCardName.FoolsGold).isEmpty())
                continue;
            ...
            if (thePlayer.getCardsFromHand(DomCardName.FoolsGold).size() > 1)
                continue;
            while (!thePlayer.getCardsFromHand(DomCardName.FoolsGold).isEmpty()) {
                DomCard theGold = owner.getCurrentGame().takeFromSupply(DomCardName.Gold);
                if (theGold == null)
                    break;
                DomCard theFool$s_GoldCard = thePlayer.getCardsFromHand(DomCardName.FoolsGold).get(0);
