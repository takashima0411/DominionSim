...
public class CopperCard extends DomCard {
    ...
    public boolean wantsToBePlayed() {
        if (owner == null) {
            log.warn("Copper owner = null: {}", DomEngine.currentPlayer.getDeck());
            log.warn("opp deck: : {}", DomEngine.currentPlayer.getOpponents().get(0).getDeck());
        }
    ...
    private boolean handlePossibleGrandMarketBuy() {
        ...
        if (owner.getCurrentGame().countInSupply(DomCardName.GrandMarket) == 0
        ...
        if (theDesiredCard != null && theDesiredCard.getTrashPriority(owner) >
                DomCardName.GrandMarket.getTrashPriority(owner))
            return false;
        ...
        if (owner.getDesiredCard(theCurrencyWithoutCoppers, false) == DomCardName.GrandMarket)
            return true;
