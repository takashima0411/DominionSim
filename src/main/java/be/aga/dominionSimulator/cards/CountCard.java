...
public class CountCard extends DomCard {
    ...
    public void play() {
        ...
        if (!owner.getCardsFromHand(DomCardName.TreasureMap).isEmpty() && owner.countInDeck(DomCardName.TreasureMap) > 1) {
            owner.putOnTopOfDeck(owner.removeCardFromHand(owner.getCardsFromHand(DomCardName.TreasureMap).get(0)));
