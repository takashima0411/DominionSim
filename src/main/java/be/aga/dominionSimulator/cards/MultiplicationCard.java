...
public class MultiplicationCard extends DomCard {
    ...
    public void play() {
        ...
        if (theCardToMultiply.getName() != DomCardName.Tactician) {
            ...
            if (getName() == DomCardName.KingsCourt) {
        ...
        for (int i = 0; i < owner.getCardsInHand().size(); i++) {
            ...
            if (theCard.hasCardType(DomCardType.Action) && theCard.wantsToBePlayed()) {
                if (theCard.hasCardType(DomCardType.Terminal)
                    ...
                    if (theCard.getName() != DomCardName.TradingPost)
    ...
    private DomCard fixForKingsCourtRabble(DomCard theCardToPlay) {
        ...
        if (owner.getActionsLeft() == 0 && owner.getCardsFromPlay(DomCardName.KingsCourt).size() == 1 && theCardToPlay != null && theCardToPlay.hasCardType(DomCardType.CardAdvantage) && theCardToPlay.hasCardType(DomCardType.Terminal)) {
            for (int i = 0; i < owner.getCardsInHand().size(); i++) {
                DomCard theCard = owner.getCardsInHand().get(i);
                if (theCard.hasCardType(DomCardType.Action) && theCard.wantsToBePlayed()) {
                    if (!theCard.hasCardType(DomCardType.CardAdvantage)
    ...
    public int getPlayPriority() {
        ...
        for (DomCard theCard : owner.getCardsInHand()) {
            if (theCard == this)
                continue;
            if (theCard.getName() == DomCardName.KingsCourt
                    || theCard.getName() == DomCardName.ThroneRoom
    ...
    public boolean wantsToBePlayed() {
        ...
        for (DomCard theCard : owner.getCardsInHand()) {
            if (theCard == this
                    || theCard.getName() == DomCardName.KingsCourt
                    || theCard.getName() == DomCardName.ThroneRoom
                    || theCard.getName() == DomCardName.Crown
