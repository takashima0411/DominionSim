    ...
    public boolean wantsToBePlayed() {
        if (owner.getPlayStrategyFor(this) == DomPlayStrategy.MarketSquareCombo) {
            if ((owner.countInDeck(DomCardName.Madman) > 4 || owner.getCardsInHand().size() > 5)
                    && owner.countInDeck(DomCardName.MarketSquare) - owner.countInPlay(DomCardName.MarketSquare) > 5 && owner.getDeck().getDeckAndDiscardSize() > 3)
                return true;
            else if (owner.getCardsInHand().size() > 10 && owner.getDeck().getDeckAndDiscardSize() > 3)
                return true;
            else
                return false;
        }
