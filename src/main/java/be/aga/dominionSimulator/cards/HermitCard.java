...
public class HermitCard extends DomCard {
    ...
    public void play() {
        if (owner.getPlayStrategyFor(this) != DomPlayStrategy.MarketSquareCombo || moreThanOneCardToTrash()) {
            ...
            if (owner.getPlayStrategyFor(this) == DomPlayStrategy.MarketSquareCombo && owner.getCardsFromHand(DomCardName.MarketSquare).size() > 4)
                maybeTrashACard();
        }
    ...
    private void maybeTrashACard() {
        ...
        if (!theCardsInHand.isEmpty()) {
            ...
            if (i < theCardsInHand.size()
                    && (theCardsInHand.get(i).getTrashPriority() <= DomCardName.Copper.getTrashPriority(owner) || (owner.getPlayStrategyFor(this) == DomPlayStrategy.MarketSquareCombo) && !owner.getCardsFromHand(DomCardName.Madman).isEmpty() && owner.getCardsFromHand(DomCardName.MarketSquare).size() > 4))
