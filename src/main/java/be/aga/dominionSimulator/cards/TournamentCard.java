...
public class TournamentCard extends DomCard {
    ...
    public void play() {
        ...
        if (!owner.getCardsFromHand(DomCardName.Province).isEmpty()) {
            if (DomEngine.haveToLog)
                DomEngine.addToLog(owner + " reveals a " + DomCardName.Province.toHTML() + " and may gain a Prize");
            gainPrize();
    ...
    public int getPlayPriority() {
        if (owner.getCardsFromHand(DomCardName.Province).isEmpty() && !owner.getCardsFromHand(DomCardName.HuntingParty).isEmpty())
            return owner.getCardsFromHand(DomCardName.HuntingParty).get(0).getPlayPriority() + 1;
