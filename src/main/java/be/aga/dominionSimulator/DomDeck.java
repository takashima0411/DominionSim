...
public class DomDeck extends EnumMap<DomCardName, ArrayList<DomCard>> {
    ...
    public void gain(DomCard aCard, int aLocation) {
        ...
        if (aLocation == DISCARD && aCard.getName() != DomCardName.Villa) {
            if (aCard.getName() == DomCardName.NomadCamp
                    || (!owner.getCardsFromPlay(DomCardName.RoyalSeal).isEmpty() && aCard.getDiscardPriority(1) >= 16)
        ...
        if (aCard.hasCardType(DomCardType.Action) && owner.getCurrentGame().getBoard().isLandmarkActive(DomCardName.DefiledShrine)) {
            owner.getCurrentGame().getBoard().moveVPFromTo(aCard.getName(), DomCardName.DefiledShrine);
        }
...
//            	.append( owner + " shows this deck (");
        ...
        if (get(DomCardName.DistantLands) != null) {
            int theCount = 0;
            for (DomCard theCard : get(DomCardName.DistantLands)) {
                theCount += theCard.getVictoryValue();
            }
            if (theCount > 0) {
                theMessage.append(", ");
                theMessage.append(get(DomCardName.DistantLands).size()).append(" ").append(DomCardName.DistantLands.toHTML());
